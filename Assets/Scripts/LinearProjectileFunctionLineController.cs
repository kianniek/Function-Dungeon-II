...
public class LinearProjectileFunctionLineController : MonoBehaviour
{
    ...
    [SerializeField, Min(0.001f)] private float c = 1f;
    ...
    [Header("Variables")]
    [SerializeField, Min(0.0001f)] private float timeStep = 0.1f; // Time step for calculating trajectory points
    ...
    {
        return new Vector2(1, a).normalized * ActiveProjectile.Speed;
    }
    ...
    {
        var clampedFollowDistance = Mathf.Clamp(ActiveProjectile.DelayValue, 0f, FunctionLineLength);
    ...
    {
        return ProjectilePhysics.CalculateGravityTrajectory(direction, ActiveProjectile.Speed, ActiveProjectile.AppliedGravity);
    }
    ...
    private float CalculateTimeToGround(float initialHeight, float initialVerticalVelocity)
    {
        ...
        var discriminant = (initialVerticalVelocity * initialVerticalVelocity) -
                           4 * ActiveProjectile.AppliedGravity * (initialHeight - groundLevel);
        ...
        var timeToGround = (-initialVerticalVelocity - Mathf.Sqrt(discriminant)) /
                           -Mathf.Abs(ActiveProjectile.AppliedGravity);
    ...
    private void UpdateTrajectoryLineRenderer(Vector3 startPosition, Vector2 initialVelocity, float timeToGround)
    {
        ...
        for (var i = 0; i <= calculatedSegments; i++)
        {
            ...
            var displacement = new Vector3(
                ...
                (initialVelocity.y * time) + 0.5f * (-Mathf.Abs(ActiveProjectile.AppliedGravity) * (time * time)),
                0);
    ...
    private void OnValidate()
    {
        if (!EditorApplication.isPlaying) 
            ActiveProjectile = debugProjectile;
